...
def runmoonbot():
    ...
    async def on_ready():
      print("bot is ready")
      print("Closing this program will cause the bot to go offline")
...
def start():
    ...
    window = Tk()
    window.title("Run")
    label = Label(window, text="Enter your bot token to run Muneeb's bot's code through your bot (see source code on github):  ")
    label.grid(row=0, column=0, sticky=N)
    entry = Entry(window, width=86, bg="light green")
    entry.grid(row=1, column=0, sticky=E)
    button = Button(window, text="RUN", command=runmoonbot)
    button.grid(row=2, column=0, sticky=S)
    label2 = Label(window, text="May not be 100% secure (don't give discord bot admin)")
    label2.grid(row=3, column=0, sticky=S)
    label3 = Label(window, text="The bot won't run if incorrect or no token is entered")
    label3.grid(row=4, column=0, sticky=S)
    window.mainloop()
